...
@register_voxelnet
class VoxelNet_track(nn.Module):
    def __init__(self,
                 output_shape,
                 num_class=2,
    ...
    def network_forward(self, voxels_batch, num_points_batch, coors_batch, batch_size_list, examples):
        ...
        for voxels, num_points, coors in zip(voxels_batch, num_points_batch, coors_batch):
            voxel_features_list.append(self.voxel_feature_extractor(voxels, num_points,
                                                        coors))
        self.end_timer("voxel_feature_extractor")

        self.start_timer("middle forward")
        for voxel_features, coors, batch_size in zip(voxel_features_list, coors_batch, batch_size_list):
            spatial_features_list.append(self.middle_feature_extractor(
                voxel_features, coors, batch_size))
        self.end_timer("middle forward")
        self.start_timer("rpn forward")
        preds_dict = self.rpn(spatial_features_list, examples)
    ...
    def forward(self, examples):
        ...
        for example in examples:
            ...
            batch_size_dev_batch.append(batch_size_dev)
        # features: [num_voxels, max_num_points_per_voxel, 7]
        # num_points: [num_voxels]
        # coors: [num_voxels, 4]
        preds_dict = self.network_forward(voxels_batch, num_points_batch, coors_batch, batch_size_dev_batch, examples)
        # need to check size.
